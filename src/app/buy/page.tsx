...
export default function Home() {
  ...
  const lotsMapped = acceptedLots.map((lot) => {
    return (
      <Button
        variant={`${
          currLots.includes(lot.toLowerCase()) && !all && !isOnSpot
            ? "default"
            : "secondary"
        }`}
        key={lot}
  ...
  const listingsMap = listings
    ...
    .map((listing) => {
      ...
      return (
        <Card
          className={`${
            isOnSpot ? "justify-center h-[300px] w-[300px]" : "h-fit w-fit"
          ...
          <CardHeader>
            ...
            {isOnSpot && (
              <EditSheet
                ...
                lot={listing.spotnumber}
                fromDate={start_date}
                toDate={end_date}
                lotName={listing.lot}
                listing_id={listing.spaceid}
              />
            )}
            {isOnSpot && (
              <Delete listing_id={listing.spaceid} setListings={setListings} />
            )}
  ...
  return (
    <div className="w-full flex flex-col md:flex-row p-2">
      ...
      <div className={`grid w-full h-fit place-items-center`}>
        <div
          className={`grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-2 gap-8 p-12 ${
            isOnSpot ? "!grid-cols-1" : ""
          }`}
        >
          {listingsMap}
