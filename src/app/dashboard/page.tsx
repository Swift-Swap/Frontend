...
export default function Dashboard() {
    ...
    const metric_end = new Date(parseInt(metric_end_split[0]), parseInt(metric_end_split[1]), parseInt(metric_end_split[2]));
    ...
    const listingsMap = mockListings.map((listing) => {
        ...
        const start = new Date(parseInt(start_split[0]), parseInt(start_split[1]), parseInt(start_split[2]));
        const end = new Date(parseInt(end_split[0]), parseInt(end_split[1]), parseInt(end_split[2]));
        const sold = new Date(parseInt(sold_split[0]), parseInt(sold_split[1]), parseInt(sold_split[2]));
        return (
            <div className="p-4 flex flex-col border-4 rounded-2xl justify-between items-center w-full mb-4 border-[#B7B7B7]" key={listing.spaceid}>
            ...
            <div className="flex flex-col flex-1 w-screen p-6 items-center md:items-start md:px-40 lg:px-52 xl:px-60">
                ...
                <div className="rounded-3xl border-4 border-[#B7B7B7] w-full p-10 flex justify-around mb-6">
                    <div className="flex flex-row gap-2 items-start">
                        <h4 className={`text-6xl font-bold ${outfit.className}`}>{stats.spots_sold}</h4>
                    ...
                    <div className="flex flex-row gap-2 items-start">
                        <h4 className={`text-6xl font-bold ${outfit.className}`}>{stats.spots_bought}</h4>
                        <div className="uppercase tracking-widest mt-2 text-xs">
                            spots
                            <br />
                            bought
                    ...
                    <div className="flex flex-row gap-2 items-start">
                        <h4 className={`text-6xl font-bold ${outfit.className}`}>{formatter.format(stats.total_revenue)}</h4>
                        <div className="uppercase tracking-widest mt-2 text-xs">
                            made
                            <br />
                            from
                    ...
                    <div className="flex flex-row gap-2 items-start">
                        <h4 className={`text-6xl font-bold ${outfit.className}`}>{stats.days_of_parking}</h4>
                        <div className="uppercase tracking-widest mt-2 text-xs">
                            days
                            <br />
                ...
                <h3 className="uppercase text-center tracking-widest mb-2"> Spots Metrics / Info </h3>
                <div className="rounded-3xl border-4 border-[#B7B7B7] w-full p-10 flex justify-around mb-6">
                    <div className="flex flex-col gap-2 items-start">
                        <h4 className={`text-6xl font-bold w-full text-center ${outfit.className}`}>{metrics.views}</h4>
                        <div className="uppercase w-full text-center tracking-widest mt-2 text-xs">
                            Views
                        </div>
                    </div>
                    <div className="flex flex-col gap-2 items-start">
                        <h4 className={`text-6xl font-bold w-full text-center ${outfit.className}`}>{metrics.parking_lot}</h4>
                        <div className="uppercase w-full text-center tracking-widest mt-2 text-xs">
                            parking lot
                        </div>
                    </div>
                    <div className="flex flex-col gap-2 items-start">
                        <h4 className={`text-6xl font-bold w-full text-center ${outfit.className}`}>#{formatNumber(metrics.spot_number)}</h4>
                        <div className="uppercase w-full text-center tracking-widest mt-2 text-xs">
                            spot number
                        </div>
                    </div>
                    <div className="flex flex-col gap-2 items-start">
                        <h4 className={`text-6xl font-bold w-full text-center ${outfit.className}`}>
